...
form.addEventListener("submit", function (e) {//or ("submit", (e) => {
  // A click on a form submit button – initiates its submission to the server.
  e.preventDefault();//do not submit to the Server (default Form behaviour)

  const value = parseInt(amount.value);
  //console.log(typeof value);//string if no parseInt()
  ...
  if (isNaN(value) || value <= 0 || value > 9) {
    ...
    let tempText = text.slice(0, value);//array section from 0 to value
    tempText = tempText.map(function (item) {
        return `<p class="result">${item}</p>`;//class 'result' is just for css
      }).join("");
